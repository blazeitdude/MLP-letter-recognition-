...
namespace s21 {
	...
	void	GraphNet::backPropagation(v_double& right) {
		v_double	grads;
	...
	/*
	 * я хз честно работает или нет, писал по примеру чтения весов для матрицы
	 * (может и там не работает) ¯\_(ツ)_/¯
	 */
	...
	void GraphNet::gradient(std::vector<double> &grads, const std::vector<double> &expect, size_t layer) {
	...
	void Neuron::setValue(double res) {
		value = res;
	}

	void Neuron::setWeights(const std::vector<double> &res) {
		weights = res;
	}

	double &Neuron::getWeights(size_t index) {
